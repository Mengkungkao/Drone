#!/usr/bin/env python3
import argparse, asyncio, csv, json, math, pathlib, time, yaml
...
async def mission(run, config, safety):
    ...
    async def collect():
        position_stream=drone.telemetry.position_velocity_ned(); armed_stream=drone.telemetry.armed()
        armed=False
        async for pv in position_stream:
            try: armed=await asyncio.wait_for(anext(armed_stream),.05)
            except TimeoutError: pass
            p,v=pv.position,pv.velocity; elapsed=time.monotonic()-started
