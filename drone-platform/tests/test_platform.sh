...
check_target_os ubuntu 24.04 x86_64
if check_target_os ubuntu 22.04 x86_64 2>/dev/null; then
  die 'unsupported Ubuntu release was accepted'
fi
if check_target_os debian 24.04 x86_64 2>/dev/null; then
  die 'unsupported operating system was accepted'
fi
if check_target_os ubuntu 24.04 aarch64 2>/dev/null; then
  die 'unsupported architecture was accepted'
fi
if (export ROS_DISTRO=humble; source_ros_jazzy) 2>/dev/null; then
